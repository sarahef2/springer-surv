...
```{r,cache=FALSE}
fit_opt <- RLT(Xr,Y,Censor, nmin = ns_opt, mtry = mtry_opt, nsplit = ifelse(splitgen_opt=="best",0,1),
               split.gen =splitgen_opt, ntrees = 1000, importance = TRUE, replacement = TRUE, resample.prob=1)
...
```{r,cache=FALSE}
VI <- data.frame(id=c(1:(dim(Xr)[2])),variable=substring(colnames(Xr),2),vi=fit_opt$VarImp,signature=c(colnames(Xr))%in%paste0("X",Gene_signature$X))
VI <- VI[order(VI$vi),]
VI$ord <- c(1:(dim(Xr)[2]))
...
ggplot(VI[(dim(VI)[1]-40):dim(VI)[1],],aes(x=variable,y=vi,fill=signature))+geom_col(width = 0.5,show.legend = TRUE)+scale_fill_manual(values=c("darkorange","deepskyblue"))+ylab("Variable Importance")+xlab("Top 40 Variables by Variable Importance")+coord_flip() + guides(fill = guide_legend(title = "Included in Wang et al. 76 Gene Signature"))
ggsave("Plots/Zhu_SurvAnalysis_Top40VI.png")
```
...
ggplot(melted2,aes(x=Time,y=Probability))+geom_step(aes(group=Subject,col=Level),show.legend = TRUE,size=1.5)+ylab("Probability of Survival")+xlab("log(months)")+scale_x_log10() + guides(col = guide_legend(title = paste0("ER Status/ ",substring(VI1,2) ," Level")))
