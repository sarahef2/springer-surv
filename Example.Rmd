---
title: "Random Forests for Survival Analysis and High-Dimensional Data: R Example"
...
The data is obtained from the NIH data repository, [Series GSE2034](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=gse2034) [@Wang2005]. This series represents 180 lymph-node negative relapse-free patients, and 106 lymph-node negate patients that developed distant metastasis.
...
```{r,cache=FALSE}
  ...
  param_tunings <- expand.grid(mtry = c(sqrt(p), p/3, p),
                   ...
                   split.rule = c("suplogrank","logrank"),
                   split.gen=c("best","random"))
  param_tunings$oob_err <- NA
```

We tune the model by running through all possible parameter combinations. For computational reasons, we use the out-of-bag c-index as the selection criterion. 
...
```{r, results="hide", eval = FALSE}
  ...
  for(i in 1:dim(param_tunings)[1])
  {
    ...
    fit_ops <- RLT(Xr, Y, Censor, ntrees = 20000,
                   nmin = param_tunings$nmin[i], 
                   mtry = param_tunings$mtry[i], 
                   split.gen = param_tunings$split.gen[i],
                   split.rule = as.character(param_tunings$split.rule[i]),
...
```{r, include=FALSE}
  #save(param_tunings, file = "Data\\Tunings20000.Rdata") 
  load("Data\\Tunings20000.Rdata")
```
...
```{r}
  ...
  splitrule_opt <- besttunings$split.rule
  ...
  (1-min(param_tunings$oob_err))/(1-max(param_tunings$oob_err))
```
...
```{r, eval=FALSE}
  library(parallel)
  ncores = detectCores()
  fit_opt <- RLT(Xr, Y, Censor, ntrees = 20000, nmin = nmin_opt, mtry = mtry_opt, 
                 nsplit = ifelse(splitgen_opt=="best",0,1), 
                 split.gen = as.character(splitgen_opt), 
                 split.rule = as.character(splitrule_opt),
                 importance = TRUE, ncores = ncores)
```

```{r, include=FALSE}
  #save(fit_opt, file = "Data\\FinalModel20000.Rdata")
  load("Data\\FinalModel20000.Rdata")
```
...
```{r,cache=FALSE}
  ...
  VI$color <- c(dim(VI)[1]:1) %in% c(1:40)
  head(VI)
  tail(VI)
...
```{r}
  ...
  ggplot(melted2,aes(x=Time,y=Probability)) +
    ...
    ylab("Probability of Survival")+xlab("Time (months)") +
    scale_x_log10() + 
