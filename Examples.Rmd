...
```{r}
fit_opt <- rfsrc(Surv(Y,Censor)~., data = Data, nodesize =ns_opt, mtry = mtry_opt, nsplit = nsplit_opt, ntree = 1000, importance = TRUE, samptype = "swr", sampsize = dim(Data)[1])
...
#melted2<-melted2[order(melted2$MaxVar),]
#melted2$MaxVar<-as.character(melted2$MaxVar)

ggplot(melted2[melted2$ER=="ER-",],aes(x=Time,y=Probability))+geom_step(aes(group=Subject,col=Level),show.legend = TRUE)+geom_point(aes(x=Yp,y=Probability,col=Level),na.rm = TRUE,show.legend = TRUE)+ylab("Probability of Survival")+xlab("log(months)")+scale_x_log10()+scale_color_manual(values=c("darkorange","deepskyblue"))
ggsave("Plots/Zhu_SurvAnalysis_TestPredsStratERn.png")

ggplot(melted2[melted2$ER=="ER+",],aes(x=Time,y=Probability))+geom_step(aes(group=Subject,col=Level),show.legend = TRUE)+geom_point(aes(x=Yp,y=Probability,col=Level),na.rm = TRUE,show.legend = TRUE)+ylab("Probability of Survival")+xlab("log(months)")+scale_x_log10()+scale_color_manual(values=c("darkorange","deepskyblue"))
ggsave("Plots/Zhu_SurvAnalysis_TestPredsStratERp.png")
